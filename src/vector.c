...
void* vector_append_vector(vector* v, vector* vec) {
  ...
  return memcpy(vector_get(v, v->count - vec->count), vec->_data,
                vec->_datasize);
}
...
void* vector_insert(vector* v, size_t index, void* item) {
  ...
  memcpyex(vector_get(v, index + 1), vector_get(v, index),
           v->type_width * (v->count - index));
...
void* vector_insert_vector(vector* v, size_t index, vector* vec) {
  ...
  memcpyex(vector_get(v, index + vec->count), vector_get(v, index),
           v->type_width * (v->count - index));
...
void vector_erase(vector* v, size_t index) {
  ...
  memcpy(vector_get(v, index), vector_get(v, index + 1),
         (v->count - 1) * v->type_width);
...
void vector_erase_range(vector* v, size_t begin, size_t end) {
  ...
  memcpyex(vector_get(v, begin), vector_get(v, end),
           v->type_width * (v->count - end));
